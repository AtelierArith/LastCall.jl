...
function _generate_crate_function_wrapper(func::RustFunctionSignature)
    ...
    elseif option_info !== nothing
        ...
        # Standard function wrapper. Use invokelatest so that after hot-reload
        # re-eval, callers always dispatch to the latest implementation (fixes
        # Julia 1.12+ world age when the module is refreshed from a callback).
        julia_ret_type = _rust_type_to_julia_type_symbol(func.return_type)
        if julia_ret_type === nothing
            julia_ret_type = :Any
        end

        impl_name = Symbol("_", func_name_str, "_impl")
        quote
            function $impl_name($(arg_syms...))
                func_ptr = _get_func_ptr($func_name_str)
                call_rust_function(func_ptr, $julia_ret_type, $(converted_args...))
            end
            function $func_name($(arg_syms...))
                Base.invokelatest($impl_name, $(arg_syms...))
            end
            export $func_name
        end
    end
...
function emit_crate_module_code(info::CrateInfo, lib_path::String;
    ...
    push!(lines, "    end")
    push!(lines, "    Libdl.dlsym(handle, name)")
    push!(lines, "end")
    push!(lines, "")
