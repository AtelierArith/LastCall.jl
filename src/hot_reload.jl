...
function check_for_changes(state::HotReloadState)
    ...
    # Iterate over a snapshot so we can mutate state.source_files on deletions.
    for src_file in copy(state.source_files)
        current_mtime = get_file_mtime(src_file)
        last_mtime = get(state.last_modified, src_file, 0.0)

        if current_mtime == 0.0
            # File was deleted (or became inaccessible) since last scan.
            filter!(f -> f != src_file, state.source_files)
            delete!(state.last_modified, src_file)
            changed = true
        elseif current_mtime != last_mtime
...
"""
    _refresh_crate_binding_module!(mod::Module)

Best-effort refresh for modules generated by `@rust_crate`.
Closes the old `_LIB_HANDLE` (if any) and re-runs `__init__()` so wrappers use the rebuilt library.
"""
function _refresh_crate_binding_module!(mod::Module)
    if !isdefined(mod, :_LIB_HANDLE)
        return nothing
    end
...
function enable_hot_reload_for_crate(crate_path::String;
    ...
    # Keep library naming aligned with @rust_crate default module naming.
    resolved_module_name = module_name === nothing ? snake_to_pascal(crate_name) : module_name
    lib_name = resolved_module_name

    composed_callback = (reloaded_lib_name, success, err) -> begin
        if success
            mod_sym = Symbol(resolved_module_name)
            if isdefined(module_scope, mod_sym)
                maybe_mod = getfield(module_scope, mod_sym)
                if maybe_mod isa Module
                    try
                        _refresh_crate_binding_module!(maybe_mod)
                    catch refresh_e
                        @warn "Hot reload: Failed to refresh module $(resolved_module_name)" exception=refresh_e
                    end
                end
            else
                @debug "Hot reload: Module $(resolved_module_name) not found for refresh"
            end
        end

        if callback !== nothing
            callback(reloaded_lib_name, success, err)
        end
    end

    return enable_hot_reload(lib_name, crate_path; callback=composed_callback, kwargs...)
end
